...
public class CanvasScreenShot : MonoBehaviour
{
    ...
    */
    ...
    //Events
    public delegate void takePictureHandler(Texture2D texture2D, byte[] pngArray);
    //public static event takePictureHandler OnPictureTaken;
    public static takePictureHandler OnPictureTaken;
    ...
    //takes Screenshot
    public void takeScreenShot(Canvas canvasPanel, Vector2 size, SCREENSHOT_TYPE screenShotType = SCREENSHOT_TYPE.IMAGE_AND_TEXT, bool createNewInstance = true)
    {
        StartCoroutine(_takeScreenShot(canvasPanel, size, screenShotType, createNewInstance));
    }

    private IEnumerator _takeScreenShot(Canvas canvasPanel, Vector2 size, SCREENSHOT_TYPE screenShotType = SCREENSHOT_TYPE.IMAGE_AND_TEXT, bool createNewInstance = true)
    {
        ...
        //Texture2D screenImage = new Texture2D((int)canvasPanel.pixelRect.width, (int)canvasPanel.pixelRect.height);

        //Get Image from screen
        screenImage.ReadPixels(new Rect(0, 0, Screen.width, Screen.height), 0, 0);
        //screenImage.ReadPixels(new Rect(0, 0, (int)canvasPanel.pixelRect.width, (int)canvasPanel.pixelRect.height), 0, 0);
        screenImage.Apply();
        ...
        string path = DataSetManager.instance.dataPath + "/CanvasScreenShot.png";
        ...
        {
            //OnPictureTaken(screenImage, pngBytes);
            OnPictureTaken(screenImage, new byte[] {});
        }
        //Destroy(screenImage);
    }
